jQuery.fn.wowSlider = function(F) {
                ...
                for (var aE = 0; aE < 2; aE++) {
                    ...
                    if (aA < 0) {
                        ...
                        if (Y > 0) {
                            ...
                            aB = aG < .5 ? 0 :aA - 1;
                            aC = Y * Math.abs(au.position()[aE ? "left" :"top"] - aB) / (Math.abs(aG - .5) - aH);
                        } else {
    ...
    var V = {
        ...
        array:function(am, c, f) {
            p(c, am[Math.floor(Math.random() * am.length)], .5, "easeOutElastic1", f);
        },
...
function ws_cube(k, g, a) {
    var c = jQuery, e = c("ul", a), b = k.perspective || 2e3;
    fullContCSS = {
        position:"absolute",
